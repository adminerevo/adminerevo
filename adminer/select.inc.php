		...
		if (!$rows) {
			...
			foreach ($rows[0] as $key => $val) {
				if (!isset($unselected[$key])) {
					$val = isset($_GET["columns"][key($select)]) ? $_GET["columns"][key($select)] : null;
					...
					$functions[$key] = isset($val["fun"]) ? $val["fun"] : null;
					next($select);
				}
			}
			...
			foreach ($adminer->rowDescriptions($rows, $foreign_keys) as $n => $row) {
				...
				foreach ($row as $key => $val) {
					if (isset($names[$key])) {
						...
						$link = "";
						if ((isset($field["type"]) && preg_match('~blob|bytea|raw|file~', $field["type"])) && $val != "") {
						...
						$text = isset($field["type"]) ? preg_match('~text|lob~', $field["type"]) : null;
						echo "<td id='$id'";
