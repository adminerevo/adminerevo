...
foreach ($row["indexes"] as $index) {
	if (!$_POST["drop_col"] || $j != key($_POST["drop_col"])) {
		...
		foreach ($index["columns"] as $key => $column) {
			...
			echo ($jush == "sql" || $jush == "mssql" ? "<input type='number' name='indexes[$j][lengths][$i]' class='size' value='" . h(isset($index["lengths"][$key]) ? $index["lengths"][$key] : '') . "' title='" . lang('Length') . "'>" : "");
			echo (support("descidx") ? checkbox("indexes[$j][descs][$i]", 1, isset($index["descs"][$key]) ? $index["descs"][$key] : '', lang('descending')) : "");
			echo " </span>";
			$i++;
		}
